...
public class EncodingDetect {
    ...
    public static String getJavaEncode(InputStream filePath, int fileSize) {
        BytesEncodingDetect s = new BytesEncodingDetect();
        String fileCode = BytesEncodingDetect.javaname[s.detectEncoding(filePath, fileSize)];
        return fileCode;
    }
     ...
     * 读取文件
     */
    public static String readFile(InputStream stream, int fileSize, boolean... isTrims) {
        StringBuffer buffer = new StringBuffer();

        String code = getJavaEncode(stream, fileSize);
        BufferedReader fr;
        boolean isTrim = isTrims.length > 0 ? isTrims[0] : false;
        String line_separator = System.getProperty("line.separator");
        try {
            String myCode = code != null && !"".equals(code) ? code : "UTF-8";
            InputStreamReader read = new InputStreamReader(stream, myCode);
...
class BytesEncodingDetect extends Encoding {
    ...
    public static void main(String argc[]) throws FileNotFoundException {
        ...
        for (i = 0; i < argc.length; i++) {
            if (argc[i].startsWith("http://") == true) {
                ...
                File file = new File(argc[i]);
                result = sinodetector.detectEncoding(new FileInputStream(file), (int) file.length());
            }
    ...
    public int detectEncoding(InputStream chinesefile, Integer fileSize) {
        byte[] rawtext;
        rawtext = new byte[fileSize];
        try {
